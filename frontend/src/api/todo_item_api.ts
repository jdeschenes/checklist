...
import { authenticatedPost, authenticatedGet } from "./authenticated-client";
...
export const BackendTodoItemAPI = {
  CreateTodoItem: async (
    ...
    return await authenticatedPost<CreateTodoItemResponse>(
      `/todo/${todo_name}/item`,
      r
    );
  },
  ListTodoItem: async (todo_name: string): Promise<ListTodoItemResponse> => {
    return await authenticatedGet<ListTodoItemResponse>(
      `/todo/${todo_name}/item`
    );
  },
